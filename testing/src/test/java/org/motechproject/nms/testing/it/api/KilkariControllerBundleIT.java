...
public class KilkariControllerBundleIT extends BasePaxIT {
    ...
    public void verifyFT112() throws IOException, InterruptedException {
         ...
         * is completed for an old subscription of 48Weeks Pack.
         **/
        // setup data to remove 2 messages per week configuration for 72 week
        // pack
        testingService.clearDatabase();
        ...
        // create subscription for child pack and complete it more than a week ago
        Subscription oldSubscription = subscriptionService.createSubscription(
                9999911122L, rh.hindiLanguage(), sh.childPack(),
                SubscriptionOrigin.MCTS_IMPORT);
        subscriptionService.updateStartDate(oldSubscription, DateTime.now().minusDays(344));
